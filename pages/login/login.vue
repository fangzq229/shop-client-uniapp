<template>
	<view>
		...
		<view class="login">
			<view class="logo">
				<image src="../../static/images/login-log.jpeg"></image>
			</view>
			<!-- #ifdef H5 -->
			<view class="login_from">
				<input placeholder="请输入手机号" v-model="tel" type="number" maxlength="11"
					placeholder-style="color: #515151"></input>
				<view class="codes">
					<input placeholder="请输入短信验证码" v-model="smscode" maxlength="6" type="number"
						placeholder-style="color: #515151"></input>
					<view @click="getCode" :style="{opacity: isCode == true ? '1':'0.7'} + ';background:' + colors">
						{{codeName}}
					</view>
				</view>
			...
			<!-- #ifdef MP-WEIXIN -->
			<view class="login_from">
				<button class="login_btn" open-type="getUserInfo" :style="'background:' + colors"
			...
			wxlogin() {
				...
				uni.login({
					...
					success: function(res) {
						uni.$ajax('/api/login/jscode2session', {
							code: res.code
						}, 'get').then((res) => {
